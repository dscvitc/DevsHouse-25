  ...
  return (
    <div id="about" className="flex flex-col w-full items-center justify-evenly text-white font-spaceGrotesk pt-12">
      ...
      <div className="md-108">
        <Heading title="ABOUT US" geometry="Dodecahedron"/> {/* Add margin-bottom here */}
      </div>
      ...
      <div className="flex flex-row items-center w-full mb-5 px-8 md:px-16">
        <Image src={clubLogo} alt={"Club Logo"} className="w-32 h-32 mr-4" />
      ...
      <div className="flex flex-col-reverse md:flex-row w-full items-center gap-12 px-8 md:px-24 m-8">
        <div className="relative md:w-1/2">
          ...
          <Image src={gdscimage} alt={"GDSC IMAGE"} className="relative w-full h-auto"></Image>
        </div>
        <div className="flex flex-col md:w-1/2 items-start relative"> {/* Make this div relative */}
          <div className="relative"> {/* Wrap Diamond and Glitter in a relative div */}
            <Image src={diamond} alt={"Diamond"} className="w-24 h-24 mb-4 animate-bounce" />
            <Image src={glitter} alt={"Glitter"} className="absolute top-0 right-[-10px] w-8 h-8 animate-bounce-glitter" /> {/* Adjusted position and animation */}
          </div>
          <p className="text-2xl text-gray-400">
